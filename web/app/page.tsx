  ...
  useEffect(() => {
    const processProposals = async () => {
      if (proposalSummariesData) {
        ...
        const acceptedCreateProposals = allProposals.filter(
          (p) => p[11] && p[2] === 0
        ); // accepted and is a create proposal
      ...
      <section className="pt-8 pb-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-12 text-center">
            <Link href="/articles">
              <Button className="bg-primary text-primary-foreground hover:bg-white hover:text-primary">
                View All Articles
              </Button>
            </Link>
          </div>
